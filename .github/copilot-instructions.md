...
- Group related cases of one behavior under a single top-level `Test<Symbol>` function with `t.Run("case name", ...)` subtests, not as separate `Test<Symbol>_<case>` functions. For example, prefer `TestResourceFetcher_teamRepositoryPermissions` with `t.Run("paging", ...)` and `t.Run("no teams", ...)` subtests over standalone `TestResourceFetcher_teamRepositoryPermissions_paging` and `TestResourceFetcher_teamRepositoryPermissions_no_teams` functions. Subtests give a clearer test-runner hierarchy, share the parent name in failure traces, and let callers run a single case via `-run TestX/case_name`.
