...
func newReportCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: "Work with a migration's node reports on the target (GHEC) side:\n" +
...
func newReportCreateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Request a node report for a migration from the target (GHEC) REST API.\n" +
...
func newReportStatusCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:  "Query the status of a migration's node report from the target (GHEC) REST API.",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			stage, err := resolveReportStage(stageFlag)
			if err != nil {
