	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := client.StartMigration(cmd.Context(), resp.Value.MigrationID); err != nil {
				return fmt.Errorf("migration %s created but failed to start: %w", resp.Value.MigrationID, annotateSourceAPIError(cmd.Context(), client, err, srcURL))
			}
...
func newListCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if status == "" && pageSize == 0 && after == "" && migrationListEmpty(resp.Value) {
				resp, err = client.ListMigrations(cmd.Context(), elmapi.ListMigrationsOptions{
...
func newCancelCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return render.Write(cmd.OutOrStdout(), render.MigrationCancel(resolvedMigrationID))
		},
	}
